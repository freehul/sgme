# -*- coding: utf-8 -*-
"""
care_watch.py — WorkBuddy主动关怀守护（可选组件）
==================================================

WorkBuddy是会话型 agent（无常驻进程），主动关怀走官方「短连接」路径：
对话开始 signal_pull 拉未消费信号。本脚本把短连接变成「准常驻」——
定时轮询事件游标，发现 care_* 信号即输出可读提醒，供WorkBuddy定时任务
捕获后推送给主人（兜底铁律：任何主动消息都必须同时在当前会话发一条）。

用法（脚本目录 = 部署副本 <WORKBUDDY_SKILLS_DIR>/sgme/scripts/）：
...
配合WorkBuddy定时任务：
  ...
  有输出（非「暂无」）时，WorkBuddy读取结果并：claim → 关怀主人 → ack
"""
...
def main(argv=None):
    p = argparse.ArgumentParser(description="SGME 主动关怀守护（WorkBuddy）")
