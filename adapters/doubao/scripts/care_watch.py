# -*- coding: utf-8 -*-
"""
care_watch.py — 豆包工作主动关怀守护（可选组件）
==================================================

豆包工作是会话型 agent（无常驻进程），主动关怀走官方「短连接」路径：
对话开始 signal_pull 拉未消费信号。本脚本把短连接变成「准常驻」——
定时轮询事件游标，发现 care_* 信号即输出可读提醒，供豆包工作定时任务
捕获后推送给主人（兜底铁律：任何主动消息都必须同时在当前会话发一条）。

用法（脚本目录 = 部署副本 <DOUBAO_SKILLS_DIR>/sgme-bridge/scripts/）：
...
配合豆包工作定时任务：
  ...
  有输出（非「暂无」）时，豆包工作读取结果并：claim → 关怀主人 → ack
"""
...
def main(argv=None):
    ...
    sp.add_argument("--subscriber", default="doubao-work", help="同步推进该订阅者游标")
    ...
    try:
        ...
        if a.cmd == "pull":
            ...
            while True:
                try:
                    resp = c.events_pull("doubao-work")
