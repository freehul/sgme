# -*- coding: utf-8 -*-
"""
selfcheck.py — SGME × 豆包工作 接入自检（能力矩阵 + 连通性）
=============================================================
...
from sgme_client import BASELINE_TOOLS, KEY_ENV, SGME, SGMEError, capability_report
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(description="SGME × 豆包工作 接入自检")
    ...
    if not a.static_only:
        ...
        print(f"密钥：{KEY_ENV}={'已设置' if info['agent_key_set'] else '未设置'}"
              f" ｜ SGME_ADMIN_KEY={'已设置' if info['admin_key_set'] else '未设置'}\n")
        ...
        run("⑦ events_pull（事件游标）", lambda: _count(c.events_pull("doubao-work", 5)))

        def _append_heartbeat():
            if a.no_append:
                return "skipped"
            r = c.append("doubao-work-selfcheck", "接入自检心跳（selfcheck.py）",
    ...
    print(f"SGME × 豆包工作 接入自检（{ok}/{len(results)} 通过）  基准能力 {len(BASELINE_TOOLS)} 个\n")
