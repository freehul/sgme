# -*- coding: utf-8 -*-
"""
sgme_client.py — SGME × Doubao Work（豆包工作）适配器客户端
=============================================================

豆包工作官方适配器（adapters/doubao）的运行时客户端。两层通道：
...
用法（能力面 = CLI 命令面；全部基准能力都有对应子命令）：
  ...
  python sgme_client.py append --session 2026-01-01-doubao --text "……" [--role user|assistant]
  python sgme_client.py inject --mode daily
  python sgme_client.py search "某项目关键词" --limit 5
  python sgme_client.py answer "我最近在做什么项目？"
  python sgme_client.py events-pull --subscriber doubao-work
...
def resolve_addresses(http_url: str | None = None, mcp_url: str | None = None,
    ...
    mcp = mcp_url or _env_str("SGME_MCP_URL") or cfg.get("SGME_MCP_URL") or derive_mcp_url(http)
...
def _print_matrix(_c, _a):
    ...
    print(f"SGME × 豆包工作 基准能力矩阵（{ok}/{len(rows)}）\n")
    for r in rows:
...
def _print_env_info(_c, _a):
    ...
    print("SGME × 豆包工作 客户端生效配置（不打印密钥值）")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="sgme_client.py",
        description="SGME × 豆包工作 适配器客户端（HTTP 层零依赖 + MCP 层）",
