# -*- coding: utf-8 -*-
"""SGME × 豆包工作 适配器客户端单元测试（mock 网络层，可离线跑）。
...
运行：
  python -m unittest discover -s adapters/doubao/tests -v
或（SGME 项目 venv）：
  .venv\\Scripts\\python.exe -m unittest discover -s adapters/doubao/tests -v
或：python -m pytest adapters/doubao/tests -q
"""
...
class TestAppend(NoDeployConfig):
    def test_content_first_line_format(self):
        ...
        with mock.patch.object(c._opener, "open", return_value=FakeResp({"file_id": "f1", "status": "new"})) as mo:
            ...
            self.assertEqual(body["agent_id"], "doubao-work")
...
class TestHttpCalls(NoDeployConfig):
    ...
    def test_events_pull_query(self):
        ...
        c.events_pull("doubao-work", limit=10)
        req = mo.call_args.args[0]
        self.assertIn("subscriber_id=doubao-work", req.full_url)
...
class TestMcpMethodSignatures(NoDeployConfig):
    ...
    def test_signal_clear(self):
        c, mo = self._client()
        c.signal_clear(signal_type="care_daily", subscriber_id="doubao-work")
        args, kw = self._call(mo)
        self.assertEqual(args[0], "signal_clear")
        self.assertEqual(args[1], {"signal_type": "care_daily", "subscriber_id": "doubao-work"})
        self.assertEqual(kw["scope"], "auto")
...
class TestCLI(NoDeployConfig):
    ...
    def test_append_from_file(self):
        ...
        self.assertEqual(code, 0)
        m.return_value.append.assert_called_once_with("s-1", "本轮内容", "user", "doubao-work")
    ...
    def test_new_mcp_commands_dispatch(self):
        ...
        cases = [
            ...
            (["signal-clear", "--subscriber", "doubao-work"],
             "signal_clear", (None, "doubao-work")),
