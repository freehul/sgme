...
2. 不存在 → 自动执行索引（无需询问）：
   ```powershell
   powershell -Command "Set-Location '<项目路径>'; node '<user-home>/AppData/Roaming/npm/node_modules/@colbymchenry/codegraph/npm-shim.js' index . 2>&1; Write-Host 'EXIT_CODE:' $LASTEXITCODE"
   ```
...
### 增量同步（代码变更后）
```powershell
powershell -Command "Set-Location '<项目路径>'; node '<user-home>/AppData/Roaming/npm/node_modules/@colbymchenry/codegraph/npm-shim.js' sync . 2>&1"
```
自动同步已开启时通常无需手动执行。
...
统一调用方式：
```powershell
node '<user-home>/AppData/Roaming/npm/node_modules/@colbymchenry/codegraph/npm-shim.js' --no-color <subcommand> <args>  # 在项目根目录执行
```
