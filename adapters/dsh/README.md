...
1. SGME Gateway 运行中（`http://<NAS_IP>:9910`，NAS 部署）
...
# 查 SGME 确认 L0 入库
curl http://<NAS_IP>:9910/v1/admin/sessions -H "X-API-Key: <admin-key>"
```
...
- 用户级技能：`<user-home>/.agents/skills/codegraph/SKILL.md`（可重建源：`codegraph-bridge/skills/codegraph/SKILL.md`）
