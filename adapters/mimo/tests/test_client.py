# -*- coding: utf-8 -*-
"""SGME × MiMo Desktop 适配器客户端单元测试（mock 网络层，可离线跑）。

覆盖：
- 地址三级解析（环境变量 → 部署配置 client.env → 回环默认）与 MCP 端点推导
...
运行：
  python -m unittest discover -s adapters/mimo/tests -v
或（SGME 项目 venv）：
  .venv\\Scripts\\python.exe -m unittest discover -s adapters/mimo/tests -v
或：python -m pytest adapters/mimo/tests -q
"""
...
class IdentityIsolated(unittest.TestCase):
    """默认屏蔽本机 ~/.sgme/mimo-agent.json，避免真实地址/密钥污染断言。"""

    def setUp(self):
        super().setUp()
        p = mock.patch.object(sc, "load_mimo_identity", return_value={})
        p.start()
...
def make_client(**ctor):
    """构造客户端：屏蔽真实 SGME 环境变量与本机身份文件，只注入占位密钥。"""
    with env_ctx(SGME_AGENT_KEY=KEY), mock.patch.object(
        sc, "load_mimo_identity", return_value={}
    ):
        return SGME(**ctor)
...
class NoDeployConfig(unittest.TestCase):
    """基类：默认屏蔽同目录部署配置与本机身份文件，保证测试与外机环境无关。"""

    def setUp(self):
        super().setUp()
        patcher = mock.patch.object(sc, "load_deploy_config", return_value={})
        self.addCleanup(patcher.stop)
        patcher.start()
        idp = mock.patch.object(sc, "load_mimo_identity", return_value={})
        self.addCleanup(idp.stop)
        idp.start()
...
class TestMimoIdentity(unittest.TestCase):
    def test_load_identity_file(self):
        with tempfile.TemporaryDirectory() as d:
            f = Path(d) / "mimo-agent.json"
            ...
            data = sc.load_mimo_identity(f)
            self.assertEqual(data["agent_id"], "mimo")
            self.assertEqual(data["http"], "http://10.0.0.9:9910")

    def test_load_identity_missing_and_invalid(self):
        self.assertEqual(sc.load_mimo_identity("/nonexistent/mimo-agent.json"), {})
        with tempfile.TemporaryDirectory() as d:
            f = Path(d) / "mimo-agent.json"
            f.write_text("not-json", encoding="utf-8")
            self.assertEqual(sc.load_mimo_identity(f), {})
    ...
    def test_identity_used_when_env_absent(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with mock.patch.object(sc, "load_mimo_identity", return_value=json.loads(f.read_text(encoding="utf-8"))), \
                    env_ctx():
                http, mcp, src = resolve_addresses()
                self.assertEqual(http, "http://10.0.0.9:9910")
                self.assertIn("mimo-agent.json", src)
                c = SGME()
...
class TestAppend(NoDeployConfig):
    def test_content_first_line_format(self):
        ...
        with mock.patch.object(c._opener, "open", return_value=FakeResp({"file_id": "f1", "status": "new"})) as mo:
            ...
            self.assertEqual(body["agent_id"], "mimo")
...
class TestHttpCalls(NoDeployConfig):
    ...
    def test_events_pull_query(self):
        ...
        c.events_pull("mimo", limit=10)
        req = mo.call_args.args[0]
        self.assertIn("subscriber_id=mimo", req.full_url)
...
class TestMcpMethodSignatures(NoDeployConfig):
    ...
    def test_signal_clear(self):
        c, mo = self._client()
        c.signal_clear(signal_type="care_daily", subscriber_id="mimo")
        args, kw = self._call(mo)
        self.assertEqual(args[0], "signal_clear")
        self.assertEqual(args[1], {"signal_type": "care_daily", "subscriber_id": "mimo"})
        self.assertEqual(kw["scope"], "auto")
...
class TestCLI(NoDeployConfig):
    ...
    def test_append_from_file(self):
        ...
        self.assertEqual(code, 0)
        m.return_value.append.assert_called_once_with("s-1", "本轮内容", "user", "mimo")
    ...
    def test_new_mcp_commands_dispatch(self):
        ...
        cases = [
            ...
            (["signal-clear", "--subscriber", "mimo"],
             "signal_clear", (None, "mimo")),
