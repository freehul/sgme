# -*- coding: utf-8 -*-
"""
selfcheck.py — SGME × MiMo Desktop 接入自检（能力矩阵 + 连通性）
=============================================================
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(description="SGME × MiMo Desktop 接入自检")
    ...
    if not a.static_only:
        ...
        run("⑦ events_pull（事件游标）", lambda: _count(c.events_pull("mimo", 5)))
    ...
    print(f"SGME × MiMo Desktop 接入自检（{ok}/{len(results)} 通过）  基准能力 {len(BASELINE_TOOLS)} 个\n")
