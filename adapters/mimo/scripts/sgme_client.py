# -*- coding: utf-8 -*-
"""
sgme_client.py — SGME × MiMo Desktop（小米 MiMo）适配器客户端
=============================================================

MiMo Desktop官方适配器（adapters/mimo）的运行时客户端。两层通道：
...
密钥（只读环境变量或本机身份文件，不硬编码、不落盘、不回显）：
  ...
  兜底：~/.sgme/mimo-agent.json（MiMo Desktop 身份文件：api_key / http / mcp 字段）

用法（能力面 = CLI 命令面；全部基准能力都有对应子命令）：
  python sgme_client.py health
  python sgme_client.py append --session 2026-01-01-mimo --text "……" [--role user|assistant]
  python sgme_client.py inject --mode daily
  python sgme_client.py search "某项目关键词" --limit 5
  python sgme_client.py answer "我最近在做什么项目？"
  python sgme_client.py events-pull --subscriber mimo
...
def load_mimo_identity(path: str | Path | None = None) -> dict:
    """读取 MiMo Desktop 本机身份文件 ~/.sgme/mimo-agent.json（不含密钥回显）。

    字段：api_key / http / mcp / agent_id。文件不存在或不可读返回 {}。
    仓库内永不出现该文件真值；仅部署侧本地使用。
    """
    f = Path(path) if path else Path.home() / ".sgme" / "mimo-agent.json"
    try:
        if not f.is_file():
            return {}
...
def resolve_addresses(http_url: str | None = None, mcp_url: str | None = None,
    ...
    if env_http:
        ...
        http, src = ident_http, "身份文件 ~/.sgme/mimo-agent.json"
    else:
    ...
    mcp = (mcp_url or _env_str("SGME_MCP_URL") or cfg.get("SGME_MCP_URL")
           or ident_mcp or derive_mcp_url(http))
...
class SGME:
    """SGME HTTP/MCP 客户端（MiMo Desktop适配器运行时）。"""
    ...
    def __init__(self, base_url: str | None = None, key: str | None = None,
        ...
        self.mcp_url = mcp
        self.source = src
        self.key = key or os.environ.get(KEY_ENV, "") or str(load_mimo_identity().get("api_key") or "")
        if not self.key:
            raise SGMEError(
                f"缺少密钥：请设置环境变量 {KEY_ENV}（管理员签发的 agt_* key）"
                "，或写入 ~/.sgme/mimo-agent.json 的 api_key 字段"
            )
...
def _print_matrix(_c, _a):
    ...
    print(f"SGME × MiMo Desktop 基准能力矩阵（{ok}/{len(rows)}）\n")
    for r in rows:
...
def _print_env_info(_c, _a):
    ...
    print("SGME × MiMo Desktop 客户端生效配置（不打印密钥值）")
    print(f"  HTTP 端点  : {http}")
    print(f"  MCP  端点  : {mcp}")
    print(f"  地址来源   : {src}")
    for env_name, desc in ((KEY_ENV, "agent 能力面"), (ADMIN_KEY_ENV, "写侧/管理能力")):
        print(f"  {env_name:<16}: {'已设置' if _env_str(env_name) else '未设置'}（{desc}）")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="sgme_client.py",
        description="SGME × MiMo Desktop 适配器客户端（HTTP 层零依赖 + MCP 层）",
