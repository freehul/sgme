...
def _vector_enabled(monkeypatch):
    ...
    于是 embed 不可达时 L1.5 会短路跳过冲突检测（本文件测的正是冲突裁决，必然全挂）。

    这里自备启用态：mock 掉 embed（返回固定向量），不再真连部署端点的 HTTP——
    此前这些用例是靠真连 NAS ollama 让 embed 成功的，属于隐性网络依赖。
    候选池仍由维度 OR 提供，裁决路径与生产一致。
    """
