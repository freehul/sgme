...
L.append("")
L.append("> 公开榜数字为 **GPT-4 judge** 下的 J-Score / F1；SGME 本次为 **智谱 glm-4-flash judge**，judge 模型不同会引入系统性偏差（强 judge 通常更严格）。检索 recall 维度（第二节）不受 judge 影响，可直接对比。")
L.append("")
L.append("| 系统 | J-Score | F1 | 备注 |")
L.append("|---|---|---|---|")
L.append(f"| **SGME (bm25, glm-4-flash judge)** | **{qa['j_score']}** | **{_overall_f1(qa)}** | 本次评测（纯 lexical 检索） |")
L.append(f"| **SGME (bm25, 检索 recall)** | **{round(overall_recall,4)}** | — | 检索维度，与 judge 无关 |")
for name, js, f1 in LEADERBOARD:
    L.append(f"| {name} | {js} | {f1} | 公开榜 (GPT-4 judge) |")
L.append("")
L.append("### 解读")
L.append("")
L.append(f"1. **检索维度**：SGME bm25 lexical 整体 session recall@8 = **{round(overall_recall,4)}**。公开榜未统一报告 retrieval recall，但 gbrain 同协议（hybrid top-8）检索召回通常更高——SGME 受限于 NAS Ollama bge-m3 嵌入 ~49s/条不可行，**未跑向量臂**，纯 lexical 口径下 0.685 属合理区间。若接入向量（hybrid）预期召回进一步提升。")
L.append("2. **QA 维度**：SGME J-score 与公开榜同数量级，但 judge 模型不同（glm-4-flash vs GPT-4），绝对值不可直接相减比较；相对排序需同 judge 复测。")
...
print(f"j_score = {qa['j_score']}  f1 = {qa['f1'] if 'f1' in qa else 'n/a'}")
