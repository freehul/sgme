"""甜点区精测：4000/4500/5000/5500/6000 五档 × 2 轮（qwen 关闭思考 + 强化 prompt）。"""
import sys, time
sys.path.insert(0, '.')
...
cfg['llm']['chains']['refinement'][0]['model'] = 'qwen/qwen3.5-9b'
...
sizes = [4000, 4500, 5000, 5500, 6000]
results = {}
    ...
    for rnd in range(2):
        ...
        try:
            raw_mems, provider, meta = l1.extract_l1(
                ...
                chunk_size=size, overlap=size//5, bucket_ctx=None, mem_conn=None,
            )
...
print('\n=== 汇总（qwen，每档 2 轮）===')
print(f'{"chunk":>6} {"成功率":>6} {"平均记忆":>8} {"总耗时":>8} {"块大小":>16}')
for size in sizes:
    r = results[size]
    rate = r['success'] / r['total'] * 100
    avg_mem = sum(r['mems']) / len(r['mems'])
    avg_time = sum(r['times']) / len(r['times'])
    print(f'{size:>6} {rate:>5.0f}% {avg_mem:>8.1f} {avg_time:>7.0f}s {str(r["chunk_sizes"]):>16}')
