...
>
> ⚠️ **现行架构注记（2026-09-07）**：①本文「skills.db 暂缓建库两步门」已被 B120（2026-08-29）打破——用户从产品演进角度拍板建库（skills.db 持久化 + skills_fts + 向量路，Backlog T-112），§二/§四 的「暂缓建库」表述按此理解；②B114（2026-08-28）已移除 wiki 桥接，技能真源 = git 仓 source_dirs 的 SKILL.md，wiki 不再承载技能页。现行定位：skills 与 memory/wiki **三模块平级**（见《SGME-架构设计-v1.0》§3.2）。
