...
| 根因（实测推翻初判） | 真实失败特征：json 报 `Expecting ',' delimiter` 且**报错位置黏在文本末尾**。按 Python json 语义，这是**结构未闭合**的特征（`json.loads('{"a":1')` 报同一错误码），而非缺失逗号。真实形态为括号不平衡——外层数组缺 `]`、或多出 `}`。 |
| 根因定位过程 | 新增只生成不解析的采样脚本，采集 15 轮真实 L2 输出落盘；对 3 个失败样本逐一实验：去掉尾部 `}` 无效、报错位置始终黏末尾 → 判定为结构未闭合；编写字符串状态感知的括号平衡器，3/3 救回（分别恢复 5/4/5 个动作）。 |
...
| 诚实边界 | ① 采样 n=15 单模型单机，不足以外推为全量成功率；② 括号平衡是**症状修复**——模型为何产出不平衡 JSON（是否与提示词 schema 示例含非法 JSON 注释/省略号有关）**未验证**，仅作推测；③ `sample_05` 的精确病因（多一个 `}` 经切片逻辑本可绕过）未完全定性，但括号平衡可修好；④ e2e 未真正触达 L2 解析路径（本地库场景表缺失），故"无回归"以单测+回归+真实样本为准。 |
