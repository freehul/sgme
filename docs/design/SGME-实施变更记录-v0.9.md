...
| 复测结果（19 题·同题同判分器·判分 agnes 免费档） | 旧提示词（主 run）**recall 0.618 / 判对 4 / 判错 5 / 无上下文 10** → 新提示词 **recall 0.864 / 判对 10 / 判错 3 / 无上下文 5**。逐题：**6 题由「无上下文」转判对**（含两根因题 `18dcd5a5` recall 0.00→1.00、`1de5cff2` 0.00→1.00），`26bdc477` 判错→判对；**3 题回归**（`0a34ad58` 对→错、`0bb5a684` 对→无上下文、`184da446` 错→无上下文），`0a995998` 错→无上下文、`1a8a66a6` 无上下文→判错（recall 0.25→1.00）、`2788b940` 仍错（recall 0.50→0.75）。总耗时 23.4 小时（单题墙钟被新提示词的记忆量拖长，全程零云费）。 |
| 复测遗留 | ① 1 题（`0100672e` 多会话）在评测阶段抛 `tuple index out of range`（18/19 成功）→ 疑评测台自身缺陷，已在复现定位（见 B176）；② `降级直存 35` 次（本地 9B 输出 JSON 不稳，与提示词加严有关，需观察）；③ 3 题回归需逐题归因（单采样波动 vs 系统性）。 |
