...
> ⚠️ **2026-09-10 B162 更新**：生产免费链（agnes-2.5-flash）四批大样本对照实测后，生产档位调为 **6000**。
> 本节结论**不推翻**（本地 9B、19 条/会话口径仍成立），但不再作为生产档位依据——
> 5000 恰好卡在 1.5× 超限拆分台阶上（14% 会话被拆、5.03% 字符重复），6000 用同等提取量
> 换来 6% / 1.26% 与调用数 −15%。详见 B162。
...
| 图召回客观不可激活（关键实测） | LongMemEval 直灌原始会话、不跑提炼 → `memory_stats` 为空 → `backfill_system_edges` 在此口径下产出 **0 条边**（已实测）。故图召回贡献 0，与 gbrain 自身跑法一致，公平可比。图召回「激活」指**生产库** backfill（生产库有 memory_stats，B133/B134 遗留步骤），属独立运维增强项，不污染评测。⚠️ **与生产库 backfill 无因果关系（2026-09-02 澄清）**：本评测每题在本地临时目录现场新建隔离库（`eval/longmemeval_eval.py:260 open_question_db`，跑完即弃），**全程不读写 NAS 生产库**；即便生产库已 backfill 36,572 条边，本评测每题临时库依然 0 边、分数不变。故生产 backfill **不是本评测的前置条件，也不影响评测分数**，二者是两件独立的事（后者仅为让线上实例真正消费已实现的图召回）。若要在本评测中量化图召回增益，正解是给评测台加 `--graph` 臂（在临时库内构造场景后跑 `backfill_system_edges`），而非动生产库。 |
...
### B145. LongMemEval 复测：hybrid 向量臂全量 + refined 提炼臂建成 + 评测口径方法论修正（v1.1.3，2026-09-02）

| 项 | 内容 |
|---|---|
| 背景 | 用户质疑「SGME 不跑提炼，功能不是直接费掉了么，跑评测就没意义了」——**质疑成立**。B143 的 500 题评测为「零 token 直灌原始会话」，L1 提炼 / L1.5 落库 / 场景治理 / 图召回全部未参与，测得的仅是**检索底座地板**，不是 SGME 完整能力。 |
| 方法论修正（自我纠错） | B143 曾以「零 token 直灌与 gbrain 一致，公平可比」为不跑提炼辩护——**该理由不成立**：①LongMemEval 官方协议并不禁止 ingest 阶段做 LLM 处理（mem0 即在 ingest 时做 LLM 抽取且仍在榜上）；②协议本义是「各系统用自己的方式 ingest 相同数据后回答相同问题」，跑提炼更忠实于协议；③直灌实为省钱省时的保守选择，包装成「公平」是错的。 |
| 检索臂复测（hybrid 全量 500 题） | 修 bug：`eval/longmemeval_eval.py:186` `random.random` 漏写 `()`（导致 7hCVkx 全量复测崩溃）。重跑 `eval/results/longmemeval_hybrid_rerun`（2118s，检索-only）：**整体 session recall@8 = bm25 0.6847 → hybrid 0.8426（+23.1%，绝对值 +0.1579）**。分题型提升：时序推理 +31.9%（0.5925→0.7816）、跨会话 +27.6%（0.6356→0.8108）、单会话-助手 +24.4%（0.8036→1.0）、单会话-偏好 +23.1%（0.4333→0.5333）、知识更新 +16.7%（0.8077→0.9423）、单会话-用户 +10.3%（0.8286→0.9143）。**难类型（跨会话/时序/偏好）获益最大**，印证向量臂补的是语义召回短板。 |
| refined 臂（跑完整提炼）建成 | 新增 `open_question_db_refined()` + `_resolve_sessions()`（`eval/longmemeval_eval.py`）：走 SGME 真实生产链路 `append_l0 → refine_one`（L0 原始层 → L1 提炼 → L1.5 落库 + 场景治理），召回经 `memory_sources.source_ref → raw_files.file_id → raw_files.session_key` 映射回 session，与其他臂同口径可比。关键实现：①`render_session_l0` 按 `# {ISO} {role}` 格式渲染（直灌用的 `**role:**` markdown 提炼链解析不了）；②**修日期解析坑**——LongMemEval 日期形如 `2023-05-20 (Sat)` 带空格与星期，空格会截断 `(\S+)` 正则致 header 不匹配（实测解析出 0 条消息），需先剥离空格后缀；③运行期把 `sgme.config.RAW_DIR` 重定向到每题临时目录（否则 L0 原始文件写入共享 `raw/` 造成跨题串库）。 |
| 提炼后端选型 | 默认 `--refine-backend cloud` = SGME **生产真实 refinement 链**（`agnes-2.5-flash`），实测可靠：**~3.8 记忆 + ~0.7 场景 / session**（q1：54 session → 169 记忆 + 38 场景）。`--refine-backend local`（本地 LM Studio）**当前不可用**：RTX 4080S 16GB 显存中 9B 模型已占 ~13GB 仅剩 ~3GB，12B/27B 加载不下；而 9B 在英文 session 上 L1 JSON 提取常失败（~1 记忆/session，且每次失败要耗 30-280s 重试）。 |
| refined 臂被算力卡住（未跑全量） | 提炼速率实测 **~60s/session**，每题约 50 session → **~1 小时/题**；全量 500 题 = 25,112 session ≈ **8 天**，不可行。**决策（用户选方案 C）：不跑 refined 全量**，先交付检索底座基线，refined 臂保持「已建成 + 已验证 + 待算力」状态并写入文档。 |
| q1 单点观察（不构成结论） | direct-bm25 与 refined 在 q1 上**同为 recall@8 = 0.0**（q1 = "What degree did I graduate with?"，single-session-user 难题）。已排除映射 bug（用真实 `make_cfg` + `init_fts` 复现确认检索路径正常，0.0 为真实结果）。**单题无结论力**，提炼是否提升召回需跨 5 题型多样本验证；文档已标注「不得引用 q1 作为任一方证据」。 |
| hybrid 臂 QA 全量（补齐可对比 J-score） | hybrid 臂此前从未带 `--qa` 跑过，补跑 `eval/results/longmemeval_hybrid_qa`（500 题，5203s，judge 智谱 glm-4-flash，errors=3 即 0.6%）：**J-score=0.384（bm25 基线 0.354，+3.0 pp）、token-F1=0.2783（基线 0.2473，+3.1 pp）、NO CONTEXT 率 0.348（基线 0.45，-10.2 pp）**；分题型 J-score：单会话-助手 0.8750 / 单会话-用户 0.6957 / 知识更新 0.4675 / 单会话-偏好 0.2667 / 跨会话 0.2273 / 时序推理 0.1579。检索 recall@8 与检索-only 复测**完全一致（0.6847 / 0.8426）**，可复现。 |
| ⚠️ 关键发现：检索/QA 剪刀差（决定下一步方向） | 检索 recall@8 提升 **+23.1%**，J-score 却只提升 **+3.0 pp**——新召回的 15.8 pp 检索量仅约 1/5 转化为正确答案。分题型互相印证：**时序推理 recall 0.7816 但 J-score 仅 0.1579；跨会话 recall 0.8108 但 J-score 仅 0.2273**（该两类合计 265 题，占全量 53%，是拉低总分的主因）。**结论：SGME 在 LongMemEval 上的瓶颈已从「检索召不回」转移到「召回了但用不对」——即多跳聚合与时序比较环节。** 继续优化检索的边际收益已很低，下一步应投向答案生成 / 跨 session 聚合 / 时序推理逻辑。此洞察已写入报告解读第 3 条与 README。 |
...
### B150. wheel 打包缺程序资源致 `pip install .` 启动即崩修复（T-142 / P0，v1.1.4，2026-09-03）

| 项 | 内容 |
|---|---|
| 背景 | 笔记本裸部署实测（v1.1.3）：`pip install .` 后启动即 `FileNotFoundError: 配置文件不存在: ...\site-packages\config\llm.yaml`（config.py:379 ← load_llm_config ← load_config ← app.py:590）。**任何非源码安装形态（wheel/sdist/PyPI）必崩**，属分发链路 P0。 |
| 根因 | `PROJECT_ROOT = Path(__file__).parent.parent` 在 wheel 下指向 `site-packages`；而 pyproject `[tool.setuptools.packages.find] exclude` 排除 `config*`/`registry*`/`templates*`/`prompts*` 且**无 `package-data` 声明** → 实测 wheel 内 sgme 包 **0 个 .yaml/.txt**。Docker 不受影响（容器 COPY 源码布局）。 |
| 方案 | Backlog T-142 三选一中的**选项①（推荐）**：资源迁入包内 + 声明 package-data。取它是因为「随包分发、importlib 可达」是唯一能同时满足 Docker / 源码开发 / 只读安装三种形态的解，②③（data-files / 代码三级回退）会留下路径歧义。 |
| 实现 | ①`config/ registry/ templates/ prompts/` → `sgme/resources/`（git 识别为 **R100 纯重命名**，内容零改动）；②pyproject 加 `[tool.setuptools.package-data] sgme = ["resources/**/*.yaml", "resources/**/*.txt"]`，并清掉失效的 exclude；③config.py 新增 `RESOURCE_ROOT`/`BUNDLE_*` 常量与 `_config_overlay_dir()`：`SGME_HOME/config`（Docker/NAS）→ 仓库根 `config`（源码开发零回归）→ `~/.sgme/config`（只读安装），`SECRETS_FILE` 同步走 overlay（`.env` 仍留仓库 `config/`，**不入包**）；④写路径加**哨兵重定向**：仅当 `path is None 且 DEFAULT_* == BUNDLE_*` 才落盘 overlay，保住既有测试 monkeypatch `DEFAULT_LLM_CONFIG`/`DEFAULT_PROVIDERS_CONFIG` 的契约；⑤`sgme.yaml` 在 overlay 缺失时回退 `BUNDLE_SGME_CONFIG`。 |
| 联动 | Dockerfile 删 4 条 `COPY`（资源已随 `COPY sgme/ sgme/` 进镜像）；`docker/entrypoint.sh` 改**循环物化** llm/providers/sgme 三个 yaml 到 `$SGME_HOME/config`（原只物化 sgme.yaml，改为三件齐全，首次启动可即得可编辑副本）；`sgme/profile/template.py`、`sgme/prompts/manager.py`、`eval/loader.py` 路径改 `RESOURCE_ROOT`；6 个测试文件的 `PROJECT_ROOT/{config,registry,templates}` 断言同步改 `RESOURCE_ROOT`。 |
| 验证 | ①wheel `sgme-1.1.4-py3-none-any.whl` **599KB / 156 文件**，`unzip -l` 确认 `sgme/resources/config/llm.yaml` **已打包**（修复前 0 个 yaml/txt）；②解压到临时目录模拟 `pip install` 后的 site-packages 布局（无仓库根、无 pyproject）→ `load_llm_config()` 正常返回（chains=['refinement']）、dimensions=14、templates/prompts 均可解析，**崩溃消除**；③定向测试 **203 passed**：test_config_home/test_l15_prescreen/test_profile/test_operations_template/test_routes_templates=118，test_routes_llm/test_config_api=44，test_operations_config=41；④源码态回归：`_config_overlay_dir()` 命中仓库根 `config/`，行为与改动前一致。 |
| ⚠️ 踩坑 | ①`git mv config sgme/resources/config` 报 `fatal: renaming 'config' failed: No such file or directory`（目标路径不存在 + 源目录含 gitignored `.env`），改用 `mv` + `git add -A`，git 自动识别为 `R100` 重命名，无数据丢失；②wheel 产物检查用 `/tmp/...` 路径在 Git Bash 下 `unzip` 报找不到文件，改用显式 `D:/tmp/sgme_wh`；③构建用 venv 缺 pyyaml，导入自检需用已装依赖的解释器跑。 |
| 遗留补验（2026-09-03 当晚，同日闭环） | ①**sdist ✅ 已验**：`python -m build --sdist` → `sgme-1.1.5.tar.gz`（324 条目，含全部 24 个资源文件、无 `.env` 泄漏），建干净 venv 安装（含全部依赖）后从仓库外目录跑门检，**19 项全 OK**：资源根落在 site-packages 包内、llm chains=refinement、providers=4（deepseek/agnes/siliconflow/nvidia）、dimensions=14、sgme.yaml 15 键、templates 4 个、prompts 5 个 txt+manifest；②**只读安装分支 ✅ 已实测**：overlay 正确回退 `~/.sgme/config`，且 bundle 兜底读到 4 个 providers；③**写入契约 ✅**：`SGME_HOME` 下写入落 overlay、回读一致，既有 `embedding` 段被保留、`providers` 整体替换（与改动前语义一致）；④门检脚本沉淀为 `scripts/oneoff/verify_installed_resources.py`（含「必须在仓库外运行 + 非 editable 安装」两道前置断言，防被源码目录掩盖打包缺陷），wheel/sdist 通用，**建议纳入发版流程**。⑤版本号 **1.1.4 → 1.1.5**（B148 铁律：同版本号提交不触发 NAS 重建）。⑥**待 NAS 生产部署验证**：镜像重建后需确认 entrypoint 物化出 llm/providers/sgme 三份 yaml，且旧卷内已有配置不被覆盖。 |
| 关联 | Backlog T-142（✅ 已解决）；顺带修 Backlog **编号冲突**——「LongMemEval 跨 session 聚合」任务与本条同号 T-142，已改号 **T-149** 并同步其外部引用 `docs/eval/longmemeval_refined_cost_v0.1.md` §6.3。 |

### B151. T-142 上生产的三道关卡修复（v1.1.5 → v1.1.6，2026-09-04）
...
### B155. T-155 MCP 9913 周期性僵死根治：同步工具移出事件循环 + 纯 ASGI 鉴权中间件 + 双探针 healthcheck（v1.1.8，2026-09-08）

| 项 | 内容 |
|---|---|
| 背景 | MCP 9913 周期性僵死（TCP 通、应用层不回包，被客户端真实使用后数秒~数分钟内僵死，重启后约 19-25 分钟复发；2026-09-07/08 多次实锤，TRAE 验证 skill_search 实测 19.7s 期间整个端口冻结）。09-08 源码级证据链定谳三条根因：①MCP SDK 1.29.1 `func_metadata.call_fn_with_arg_validation` 对**同步工具直接在事件循环上执行**（`fn_is_async=False → return fn(**args)`，无线程池）——SGME 39 个 MCP 工具全是同步函数做 SQLite I/O；②`ApiKeyMiddleware` 继承 starlette `BaseHTTPMiddleware`，与 SSE 长流（streamable-http 全部响应是 `EventSourceResponse`）组合是社区已知死锁高危形态；③docker healthcheck 只探 9910，MCP 独立线程僵死时容器仍 healthy，自动重启永不触发。 |
| 改动 | ①`sgme/mcp_server.py` 新增 `tool()` 包装器：同步工具经 `anyio.to_thread.run_sync`（CapacityLimiter 20，惰性创建——CapacityLimiter 绑定事件循环不能模块级实例化）丢线程池，`functools.wraps` 保签名/annotations/__wrapped__（FastMCP `_is_async_callable` 判 async、`find_context_parameter` 经 get_type_hints 跟踪 `__wrapped__` 照常注入 Context）；39 处注册点改 `@mcp.tool()` + `@tool` 叠加；②`ApiKeyMiddleware` 重写为纯 ASGI（`__call__(scope, receive, send)`）：403 直接经 send 下发 JSON（响应体与原 BaseHTTPMiddleware 版逐字节同语义），key 写入 `scope["state"]["api_key"]`（Starlette Request.state 与 scope state 共享同一 dict，工具内 `ctx.request_context.request.state.api_key` 反查零改动）；③`docker-compose.yml` + `deploy/nas-docker-compose.yml`：healthcheck 改双探针（9910 health 200 且 9913 无 key POST /mcp 秒回 403；探针逻辑先经本地失败态/NAS 真实端点两方向实测再落盘）+ `cap_add: [SYS_PTRACE]`（容器内 py-spy 抓栈取证）；NAS 模板顺带对齐 B123 漏同步的 healthcheck timeout（3s/5s → 25s/30s）。 |
| 测试 | RED→GREEN：新增 3 用例（`test_mcp_server.py` 尾部 T-155 段）——`test_mcp_sync_tools_wrapped_off_event_loop`（抽样 4 工具断言注册后为协程函数 + `__wrapped__` 保留原同步实现 + `__name__` 无漂移；断言走 `mcp._tool_manager._tools` 内部注册表，协议层 `list_tools()` 的 Tool 模型不含 fn）、`test_mcp_tool_call_still_works_after_wrap`（直调层 health/stats/refine_status 可解析无 error）、`test_mcp_signal_claim_ctx_injection_survives_wrap`（HTTP 全链：注册 key 调 signal_claim → claimed=true 且 agent_id=注册名，守卫 Context 注入/PR#2 反查语义）；回归 `test_mcp_server.py` 35 passed + `test_mcp_wiki.py` + `test_entry_hardening.py` 25 passed。 |
| 排障认知修正 | ①MCP uvicorn `log_level="warning"` 吞访问日志——「僵死期日志无请求行」不能作为请求未到达的证据（此前误判过）；②py-spy 容器内抓栈需 SYS_PTRACE（本次 compose 已加，下一镜像生效；过渡期用 NAS 宿主机 `/tmp/sgme-spy/py-spy` 抓宿主 PID）。 |
| 运维影响 | 版本 bump 1.1.7 → 1.1.8（updater 以版本号为唯一触发器，同版本热修不生效——B154 踩坑确认）；部署走正规链：push → GitHub Actions 同步 Gitee → NAS updater build 部署；NAS compose 需一次性外科手术对齐（cap_add + healthcheck 双探针；updater 只 sed image 行，其余字段不自动同步）；部署后 TRAE/WorkBuddy 等 MCP 客户端需禁用→启用重新 initialize（容器重建旧会话 404）；预期效果：单个慢工具不再冻结端口、中间件死锁面消除、MCP 僵死 90s 内被 healthcheck 自动重启（30s interval × 3 retries）。 |
| 部署实录（2026-09-08 当日） | 全链 ~15 分钟：push e9d3770 → Sync to Gitee 15s 成功 → NAS 裸仓手动 fetch（T-154 缺口补偿：fetch gitee main + merge-base FF 校验 + update-ref 推进 main，0bc62e1→e9d3770）→ 意图文件 request.json 写入 → updater cron 拾取自动 build+部署（18:31 版本确认 1.1.8）→ compose 外科对齐 cap_add+healthcheck → **容器 healthy、版本 1.1.8、CAP_SYS_PTRACE 生效**。⚠️ 部署踩坑一：**群晖 compose 执行链会把 YAML 多行标量折叠成单行**——healthcheck 双探针多行 Python 版（`\n` 转义）NAS 实跑 SyntaxError 致容器 unhealthy，本地 docker compose config 验证发现不了（本地引擎不折叠）；改单行版（http.client 低层读状态码，异常自然非零退出）后真值表验证（双活 0/僵死 1）+ NAS healthy；**healthcheck 内嵌 Python 必须保持单行**已写回两份 compose 注释（fix 提交 9661689）。⚠️ 踩坑二：CapAdd 字段名——YAML 键 `cap_add`（docker compose），docker inspect JSON 路径 `.HostConfig.CapAdd`（值 `CAP_SYS_PTRACE`）。生产端到端终验：9910 health 200 (v1.1.8) + 9913 探针 0.00s 403 + agent key initialize 200 + skill_search 0.3s（TRAE 当时 19.7s）+ skill_search 执行期间 3 并发探针全部 403 0.0~0.02s（事件循环不再被工具阻塞，核心修复生效实证）。 |
| 关联 | 待办池 0569ef53（MCP 9913 周期性僵死根治）；Backlog T-155；skills 分册 `sgme-operations/references/gateway-ops.md`（2026-09-08 二次复发处置 + 根因证据链）。 |

### B156. skills_fts 外部内容表缺 name_seg 列修复：技能全文检索路生产不可用（T-112 遗留缺陷，2026-09-08）

| 项 | 内容 |
|---|---|
| 背景 | 补录 docker-buildx-ops 技能后追查「技能已入库（406→407）但搜索不召回」，生产实证（NAS v1.1.8 容器内探查）：`skills_fts`（FTS5 外部内容表，`content='skills'`）定义含 `name_seg` 列，但 `skills` 主表漏建该列（本地 HEAD 的 `SKILLS_DDL` 同样没有——T-112 B120 建库时引入，此后每次搜索的 FTS 路都异常）。三重症状：①技能 UPDATE/DELETE 触发器 `no such column: T.name_seg`（更新即炸）；②任何 FTS 全表扫描（`SELECT count(*) FROM skills_fts`、rebuild）同炸；③触发器 skills_ai/au 把未分词的 `new.name` 塞进 name_seg 位——BM25 的 10× 名字加权形同虚设，技能检索实际全靠向量路硬撑（同时解释 WorkBuddy 审计「搜 docker 零命中」的召回偏弱）。 |
| 改动 | ①`sgme/data/db.py` SKILLS_DDL：skills 表补 `name_seg TEXT` 列；②新增 `_migrate_skills_name_seg(conn)`（connect_skills 时调用，幂等）：ALTER 补列 + 回填存量（`name_seg=segment(name)`，缺分词的 description_seg/content_seg 一并按当前口径补齐，只补 NULL/空不重算），返回 bool 供调用方判断是否重建；③触发器改用真实分词列 `new.name_seg`，且 DROP TRIGGER IF EXISTS + CREATE（防旧坏触发器被 IF NOT EXISTS 短路——旧触发器函数体已坏，仅靠 IF NOT EXISTS 永不更新）；④`sgme/data/skills_dao.py` upsert_skill 写入 `name_seg=segment(rec["name"])`（INSERT 与 ON CONFLICT UPDATE 双路）；⑤connect_skills 迁移发生时 `INSERT INTO skills_fts(skills_fts) VALUES('rebuild')` 全量重建（清被污染的历史索引；407 条毫秒级）。 |
| 测试 | TDD RED→GREEN：`tests/test_skills_db.py` 新增 `TestFtsNameSegColumn` 6 用例——主表有列、name_seg 为分词串非原名、全表扫描不炸、UPDATE/DELETE 触发器守恒、名字加权命中排序（按技能名搜目标排第一）、旧库迁移回归（模拟生产无 name_seg 旧库 → connect_skills 迁移后存量可检索）。回归：skills 模块 11 文件 266 passed / 0 failed。 |
...
### B162. L1 分块档位 5000 → 6000：跨过超限拆分台阶（T-157，2026-09-10）

| 项 | 内容 |
|---|---|
| 背景 | B8（2026-08-06）以本地 9B 标定 `chunk_size: 5000` 为甜点区峰值，此后生产/评测/代码三方沿用。2026-09-10 用户在重跑 LongMemEval refined 臂前要求**重测生产免费链口径下 5000 的合理性**（怀疑档位与生产实况脱节）。 |
| 机制核实 | 代码级确认 `chunk_size` **不是固定切块**（用户质疑成立）：`refine.py:136` 读配置后交 `l1.chunk_messages_by_turn`（回合感知）——按回合累加，累到 0.9× 后下一回合装不下才落块，故**实块 ∈ [0.9×, 1.0×]**；单回合超上限独立成块（不截断）；**超 1.5× 才按消息边界拆，且带 1 条消息重叠**（`l1.py:283-299` `_split_oversized`）。→ 决定性的量不是"块多大"，而是**1.5× 这条拆台阶**。 |
| 实验设计 | 四批 + 噪声基线，全部走**生产真实链**（`llm_override` 与生产一致：agnes-2.5-flash / max_tokens 8192，链 agnes → siliconflow → rule），样本取自生产 raw 真实会话：①批次1 12 样本 × [5000, 8000, 16000]；②大样本 5 样本 × [5000, 6000, 16000]；③6000 档补跑；④噪声基线（同配置同样本独立跑两次）。工具：`tmp/chunk_size_quality_test.py` + `tmp/chunk_size_report.py`。 |
| 结果（大样本） | 5000：46 块 / **221 条** / 每块 4.80 / 提示 349,212 字符；6000：40 块 / **213 条** / 5.33 / 332,982；16000：17 块 / **107 条** / 6.29 / 271,778。逐样本单调（如 104,872 字符 → 107/106/55）。调用成功率 100%、零回退、零块失败。 |
| 噪声基线（判可信度的关键） | 同配置同样本独立两次：记忆条数绝对差均值 1.88、最大 4、相对 21.6%。分层结论：**5000 vs 6000（−1.6 条）= 噪声内，两档等价**；5000 vs 16000（−22.8 条，5/5 样本同向）= **硬结论**；5000 vs 8000（−3.14 条，7/7 同向）= 弱信号。 |
| 定论 | ①**5000 不必推翻**（B8 的甜点区结论在本地 9B 口径下仍成立），但它**恰好卡在 1.5× 拆台阶上**：230 个生产会话实测——5000 → 2.46 块/会话、**14% 会话被拆**、**5.03% 字符重复送入**；6000 → 2.10 / 6% / 1.26%；8000 → 1.76 / 3% / 1.20%。②**16000 明确不可取**：提取量腰斩是**欠提取**（模型拿大块反而"总结"掉一半信息），非"更精炼"。③大块并不更省：5000 档比 8000 档多喂 21% 提示字符、多 32% 调用。 |
| 改动 | ①**生产**：NAS 覆盖层 `/data/config/sgme.yaml` 的 `l1.chunk_size: 5000 → 6000`（**唯一真正改变生产行为的一处**——`docker/entrypoint.sh` 仅在文件缺失时物化模板，覆盖层一旦存在，改包内默认对生产零影响）；②`sgme/resources/config/sgme.yaml` 5000 → 6000（新装/无覆盖层默认，含评测环境）；③`sgme/config.py` `DEFAULT_L1_CONFIG` 8000 → 6000（最后兜底，消除 5000/8000/6000 三方不一致——`refine.py:136` 代码内默认本就是 6000）；④`sgme/engine/l1.py` `chunk_messages_by_turn` 签名默认 5000 → 6000（纯一致性，生产显式传参不受影响）；⑤`config.py` 注释补注「`overlap` 仅字符串路径 `chunk_conversation` 生效，回合路径不读」（死配置澄清，未删）。 |
| 生产生效 | 走 admin 热更新 API（`POST /v1/admin/config {"section":"l1","values":{"chunk_size":6000}}`）：`apply_section` 深层合并缺键保留（`overlap` 与 `search.graph` 等未被吃）、`update_config` 就地改内存 cfg + 落盘 → **无需重启、提炼线程即刻读新值**。改前备份 `sgme.yaml.bak-20260910`（md5 `f0462698…` 前后一致）；改后读回 `"l1":{"chunk_size":6000,"overlap":1000}`、宿主文件 md5 `d27e0fe7…`（持久化确认）。脚本 `tmp/apply_chunk6000.py`（带参可回滚）。 |
| 影响面 | 重复率 5.03% → 1.26%、拆分会话 14% → 6%、LLM 调用数 −15%、提示字符 −5%；**提取量差 −1.6 条在噪声内 → 只能说"不亏"，不是提质**，不得当质量改进宣传。免费链有限流，调用数下降同为正收益。 |
| 诚实边界 | ①6000 的收益来自"跨过台阶"（更少拆分/重复/调用），不是"模型看得更好"；②改后**不能**用"记忆条数"做回归指标（差异落在噪声内），需盯重复率与调用数；③B8 原文与既有评测成本文档（`docs/eval/longmemeval_refined_cost_v0.1.md`，chunk_size=5000 口径）**均保留不改数字**，只加按语指向本条。 |
...
### B165. T-150 评测启动器迁入 `eval/` 并入库：消除 `tmp/` 易失依赖（2026-09-11）
...
### B168. L2/L1.5 解析容错加固与归一化告警门槛（2026-09-11）

| 项 | 内容 |
|---|---|
| 背景 | 双机评测日志（27KB/331 行）暴露 L2 场景层解析大面积失败：L2 相关行 101 条中 88 条为解析失败，成功率仅约 13%。初判为"模型输出漏逗号"，据此实现"按 json 报错位置补逗号"，并在 15 轮真实 LLM 采样中验证——**该方向被数据否决**（3 个真实失败样本 0/3 救回）。 |
| 根因（实测推翻初判） | 真实失败特征是**括号不平衡**，而非缺失逗号：字符串状态感知扫描 3 个真实失败样本，2 例为**多余闭合符**（sample_05 多 5 个 `}`、sample_06 多 3 个 `]`，报错点落在文档**中途**）、1 例为**缺闭合符**（sample_10 外层数组缺 `]`，报错点落在**文本末字符**）。按 Python json 语义，结构未闭合同样报 `Expecting ',' delimiter`（`json.loads('{"a":1')` 报同一错误码），故单看错误码会误判为漏逗号。 |
| 根因定位过程 | 新增只生成不解析的采样脚本，采集 15 轮真实 L2 输出落盘（同一输入同提示词，纯测模型随机性）；对 3 个失败样本逐一实验：**去掉尾部多余 `}` 无效**（推翻了「尾部多括号」的简单猜测）→ 改用字符串状态感知的括号扫描（引号内括号不计入配平）→ 定性为括号不平衡（2 多 1 缺）→ 编写括号平衡器，3/3 救回（分别恢复 5/4/5 个动作）。**初版只做了「按报错位置补逗号」，实测 0/3 救回。** |
| 改动 1 | `sgme/engine/l2.py`：新增 `_repair_bracket_balance`（字符串状态感知扫描，丢弃多余闭合符 + 末尾补齐未闭合括号 + 补未闭合引号）、`_insert_missing_commas`（独立病因兜底）、`_loads_robust`（三级容错：原样 → 括号平衡 → 括号平衡+补逗号），替换原 `_loads_repairing_commas`。**关键**：全部失败时上抛「原始错误」，修复循环不再掩盖真实病因（原先会把病因报成误导性的 `line 1 column 1`）。 |
| 改动 2 | `sgme/engine/l15.py`：`parse_l15_output` 首段解析改用 `json.JSONDecoder().raw_decode`，容忍模型在数组后附加说明文字（原 `find("[")`/`rfind("]")` 切片在说明文字含方括号时取错区间）。 |
| 改动 3 | `sgme/engine/normalize.py`：新增 `DROP_RATE_MIN_SAMPLES = 5`，丢弃率告警与 `should_warn` 均加最小样本门槛。原逻辑在样本量 1–3 时也告警，实测 7 次告警全为小样本假阳性。既有用例 `total=5` 恰好不受影响。 |
| 测试 | `tests/test_l2.py` +3 括号不平衡用例（缺外层 `]` / 多一个 `}` / 正文含未配对括号不误判）；`tests/test_l15.py` +1 尾随内容用例；`tests/test_engine.py` +2 小样本门槛用例。RED 已实证（旧方案对"缺外层 `]`"失败）。 |
...
### B174. refined 臂会话级聚合口径 + A/B 实测：无上下文主因是检索命中率（2026-09-12）

| 项 | 内容 |
|---|---|
| 背景 | 100 题小样跑到 15 题时 `noctx` 占 67%（10/15），用户质疑「为什么有这么多无上下文」。逐题体检：**7 道 noctx 题的 gold 会话在库里全都有记忆**（1~53 条/会话）→ 不是提炼漏抽。发现 refined 库 **8~10 条记忆/场会话**，而评测按「条」取 top-k=8 → 同一 k 下 refined 臂只覆盖 1~2 场会话，直灌臂覆盖 8 场完整会话原文（信息量差 ~8 倍），口径不可比。 |
| 改动 | `eval/longmemeval_eval.py` 新增 refined 臂**会话级聚合**：先取 `top-k×10` 记忆池 → 经 `memory_sources.source_ref → file_id → session` 映射（**多来源**：一条记忆可来自多场会话）→ 按「最好一条记忆的排名」取前 N 个会话 → 只把这些会话的记忆喂 QA（字符预算可调）。新增 `_source_to_sid`/`_memory_session_map`/`_rank_sessions`/`_select_memories_within_budget` 与 `--refined-session-k`（默认=top-k；0=旧口径）/`--refined-pool`/`--refined-ctx-chars`。 |
| 测试 | `tests/test_eval_harness.py` 8 通过 / 0 失败（RED→GREEN：客户端 trust_env、断点不丢题、import 无副作用、装载档覆盖、段号剥离、多来源排序、预算内选取）。 |
| A/B 实测（同 15 题、判分 agnes 免费档、复用同一批提炼库） | 旧口径 0.550 recall / 判对 4 / noctx 9；聚合 96K 字符 0.506 / 5 / 7；聚合 48K 0.506 / **6** / 8；聚合 24K 0.611 / 5 / 5。**口径改动在 15 题噪声内（±1~2 题），上下文预算不是杠杆**。 |
| **关键发现** | 同 15 题与直灌臂 500 题留档逐题对照：**直灌臂 recall@8 = 0.983 / 判对 12**；**refined 臂 = 0.506 / 判对 5**。四道题（06878be2 / 18dcd5a5 / 1de5cff2 / 1a1907b4）直灌 recall=1.0 判对、refined recall=0.0 直接答不出。**结论：noctx 主因是 refined 臂检索命中率只有直灌臂一半**（蒸馏后的记忆文本与问题措辞语义距离变大、关键词/细节被压缩），属真实能力差距而非口径假象。 |
| 诚实边界 | ① 本轮 refined 提炼跑的是**本地 9B 蒸馏模型**（ctx 32K、L1 空重试 66+、降级直存 6），云链（agnes）质量未测；② A/B 仅 15 题、单次运行，无噪声基线；③ 口径改动保留但**报告主口径仍用旧定义**（与直灌臂同定义可比）。 |
| 未做（待定） | 定位「金标准记忆是否进了 top-80 候选池」——进了=排序问题（RRF/权重/去重），没进=提炼保真问题（提示词需保留关键实体/数字）。 |

### B175. L1 提炼提示词根因修复：事实保真版 + fact 类型 + 语言守门（2026-09-12）

| 项 | 内容 |
|---|---|
| 背景 | 用户令「先停止任务、找到问题、做对照测试，把出错题目作为对照样本」。停主 run（19/100）后取 15 道失败题 + 4 道通过题做对照，并回答 B174 遗留问题：**金标准会话的记忆确实进了 top-80 候选池，但排名 17/50** → top-8 取不到（排序层）。进一步把金标准会话原文与它被提炼出的记忆并排打印，发现**判别性细节消失**：1de5cff2「用亚马逊野生橡胶的品牌」（金标准 Veja）提炼产物只剩「用户重视可持续品牌……Veja 是值得的」；18dcd5a5「队伍面对几个木乃伊」（答案 4）整场会话只出 1 条记忆、数字与助手给的内容全丢；对照通过题 001be529 含答案细节的记忆排第 2。读 `sgme/resources/prompts/l1_extraction.txt` 定位设计层根因：提示词是**用户画像导向**（「提取值得长期保存的**用户记忆**」、类型仅 persona/episodic/instruction、「**宁缺毋滥**，临时细节不要提取」）→ **助手提供的事实没有记忆通道**，低召回是系统性设计错配，而非引擎缺陷或模型太弱。 |
| 对照实验（判据） | 同一本地 9B、同一会话、只换提示词（旧版 vs 事实保真版）：18dcd5a5 10 条/全中文/相似 0.343 → **45 条/全英文/0.536**（含 "There are 4 Mummies in the temple."）；1de5cff2 1 条/中文/0.225 → **7 条/全英文/0.587**（含 "wild rubber sourced from the Amazon rainforest"）；15745da0 0.285 → **0.672**（含 "for three months"）；06878be2 0.141 → 0.230（Sony 命中 2 → 23 条）；1a1907b4 0.272 → 0.294（**仍中文，未修复**）。结论：**提示词是主杠杆（5 题中 4 题的语言与细节被救回），模型能力是残留项**。 |
| 改动 1（提示词·版本化） | `l1_extraction.txt` 升级「事实保真版」：两类事实同等提取（用户侧 + 会话事实，含助手给出的事实）、**强制输出与会话原文同语言**、保留判别性细节（禁概括成抽象结论）、一条记忆一事实点、新增 fact 类型。旧版存为 `v004`（内容=v003），回滚一行：`PromptStore().activate("l1_extraction","v004")`。二稿（v006）把语言守则**前置到首句 + 文末再强调**并标注中文示例仅示格式——首稿实测语言守则埋在中段时 9B 仍把英文会话译成中文。 |
| 改动 2（引擎守门） | `sgme/engine/l1.py`：`VALID_MEMORY_TYPES` 增 `fact`；新增 `_lang_mismatch(conversation, memories)`——会话以英文为主而记忆以中文为主 → 判「语言漂移」，`_extract_l1_chunk` 带语言提示**重试一次**（独立预算，不改 B164 的空结果重试次数）。 |
| 测试（RED→GREEN） | 新增 `tests/test_l1_lang_guard.py` 6 例（漂移判定/原文语言不判/中文会话不判/空与短输入不判/混合不判/重试带语言提示）、`tests/test_l1_memory_types.py` 3 例（fact 入白名单、fact 被接受、未知类型仍回落 persona）；`tests/test_l1_chunk.py` 夹具改中文填充（否则误触守门、改变调用次数）。相关 12 个测试文件 **201 通过 / 0 失败**。 |
| 实测（单题速探·二稿） | 1de5cff2：旧版 2 条/中文 0/相似 0.270 → 新版 **24 条/中文 0（全英文）**/相似 **0.587**，最相似那条即 "Veja is a French brand that produces eco-friendly sneakers using organic cotton, recycled plastic bottles, and wild rubber sourced from the Amazon rainforest."（与对照实验的理想结果一致）。 |
| 运维影响 | ① **记忆条数显著增加**（同会话 2→24、5→27 等，实测最高约 19 倍）→ 存储/检索候选/注入预算需配套「控量门禁」（第二批，未做）；② NAS 生产需构建部署后生效，回滚走版本号；③ 守门重试仅漂移时触发，常态零额外调用。 |
| 复测结果（19 题·同题同判分器·判分 agnes 免费档） | 旧提示词（主 run）**recall 0.618 / 判对 4 / 判错 5 / 无上下文 10** → 新提示词 **recall 0.917 / 判对 11 / 判错 3 / 无上下文 5**。逐题：**6 题由「无上下文」转判对**（含两根因题 `18dcd5a5` recall 0.00→1.00、`1de5cff2` 0.00→1.00），**2 题由判错转判对**（`0100672e` 0.50→1.00、`26bdc477`）；**3 题回归**（`0a34ad58` 对→错、`0bb5a684` 对→无上下文、`184da446` 错→无上下文），`0a995998` 错→无上下文、`1a8a66a6` 无上下文→判错（recall 0.25→1.00）、`2788b940` 仍错（recall 0.50→0.75）、`1a1907b4` 仍无上下文。总耗时 23.4 小时（单题墙钟被新提示词的记忆量拖长，全程零云费）。 |
| 复测遗留 | ① `0100672e` 首轮抛 `tuple index out of range`（18/19 成功）→ 已定位为**向量端点瞬时拒连（WinError 10061）整批连败**，加固后 resume 补跑成功（判错→判对），见 B176；② `降级直存 35` 次（本地 9B 输出 JSON 不稳，与提示词加严有关，需观察）；③ 3 题回归需逐题归因（单采样波动 vs 系统性）。 |
| 诚实边界 | ① 会话级 A/B 是**单次采样**（温度 0.6），同题两次跑记忆条数可从 4 变到 24 → 结论需端到端复测取均值确认；② 1a1907b4 仍语言漂移，靠守门重试兜底；③ 记忆条数的增幅阈值与去重规则未定（第二批）。 |

### B176. 评测台加固：批量向量二分拆批重试 + 本机检查走死代理误报排障（2026-09-13）

| 项 | 内容 |
|---|---|
| 背景 | 19 题复测中 `0100672e`（多会话）整题作废，checkpoint 记录 `error="tuple index out of range"`。带 traceback 的探针（`tmp/probe_q.py`：patch `_process_question` 打全栈）复现后定性：真实失败是 **`batch embed 耗尽重试`**，底层 `[WinError 10061] 目标计算机积极拒绝` —— **向量端点瞬时拒连**，整批 32 条连续 6 次失败 → 直接抛错作废整题（探针连跑两次均复现；随后端点从笔记本实测可达，属抖动而非配置错误）。 |
| 改动 | `eval/longmemeval_eval.py` 新增模块级 `_embed_with_split(cli, base_url, model, batch, max_attempts=6, sleep_s=None)`：失败重试耗尽且批 >1 条时**二分拆批递归**（小批更易穿过抖动；单条超限不再拖累整批）；`embed_corpus.embed_batch` 改为薄包装。`sleep_s` 仅测试注入用。 |
| 测试 | 新增 `tests/test_eval_embed_retry.py` 3 例（整批失败→拆批后逐条成功、整批成功不拆批、单条仍抛 RuntimeError）；与 `tests/test_eval_harness.py` 8 例合计 **11 通过 / 0 失败**。 |
| 验证 | 笔记本同步后 `--resume` 补跑该题：**256 秒完成、error=null、判错→判对（recall 0.50→1.00）**，19/19 齐全（补跑只做向量+检索+判分，提炼库复用故快）。 |
