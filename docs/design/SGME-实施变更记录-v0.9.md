...
| 遗留 | ①sdist 形态未单独实测（wheel 已验，package-data 对两者同源生效，建议发版前补一次 `pip install dist/*.tar.gz` 冒烟）；②只读安装落 `~/.sgme/config` 的分支仅做路径推演，未在真正只读环境端到端实测；③本次未做 NAS 生产部署验证（改动含 Dockerfile/entrypoint，下次镜像重建时需确认 entrypoint 物化出三份 yaml 且旧卷内已有配置不被覆盖）。 |
