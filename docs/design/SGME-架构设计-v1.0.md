...
9. **LLM 降级链**：agnes（agnes-2.5-flash，免费主模型）→ siliconflow（DeepSeek-V4-Flash，免费）→ zhipu（GLM-4.7-Flash，免费兜底）→ rule drop_batch；模型名含 pro/reasoner/thinking 或命中 gemma-4-12b-qat 拒绝加载
...
chains:
  refinement:                      # 提炼管线（L1/L1.5/L2/Tier0 摘要共用）
    ...
    - provider: siliconflow        # 第二优先（硅基流动 DeepSeek-V4-Flash 免费，1-3s）
      ...
      max_tokens: 16384
    - provider: zhipu              # 末位兜底（智谱 GLM-4.7-Flash 永久免费，实测慢 38s/次 + 高峰限流）
      model: glm-4.7-flash
      max_tokens: 16384
...
- 每级模型独立窗口：agnes 200K / siliconflow 128K / zhipu 200K → budget ≈ window − 4K − 0.08×window（L1.5 分批、L2 分批、L1 长会话分块都按此）；`prompt_overhead` 为 0.08（8%）
...
- `agnes-2.5-flash` 为默认主模型（免费，当前 $0/1M token），`deepseek-ai/DeepSeek-V4-Flash`（硅基流动免费）第二、`glm-4.7-flash`（智谱永久免费）末位兜底；`gemma-4-12b-qat` 禁用（白名单 deny_exact）
