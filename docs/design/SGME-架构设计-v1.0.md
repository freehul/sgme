...
- **L1 分块（`chunk_size` 语义，B162/T-157）**：`l1.chunk_size` **不是固定切块大小**，而是回合感知的**块上界**——`chunk_messages_by_turn`（`engine/l1.py`）按回合（一条 user + 其后所有 assistant/tool）累加字符，累到 `0.9×chunk_size` 后「下一回合装不下」才落块，故**实块 ∈ [0.9×chunk_size, chunk_size]**；单回合超上界独立成块（不截断）；**超 1.5×chunk_size 才按消息边界拆**，拆分块之间带 1 条消息重叠（`_split_oversized`）；无 user 消息时按 20 条消息兜底。计量口径 = **格式化后字符**（非 raw content 裸字符，差约 1.4×）。派生量：**1.5×chunk_size 是决定性的"拆台阶"**（230 生产会话实测：5000 → 14% 会话被拆 / 5.03% 字符重复；6000 → 6% / 1.26%）。生产与评测档位 **6000**（2026-09-10 B162 由 5000 上调，理由见变更记录 B162）；`l1.overlap` **仅字符串路径 `chunk_conversation` 生效，回合路径不读**（历史遗留死配置，待清理）。
