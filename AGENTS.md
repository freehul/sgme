...
9. LLM 降级链：deepseek（deepseek-v4-flash，1M 窗口，主模型，2026-08-06 用户拍板）→ rule drop_batch；模型名含 pro/reasoner/thinking 或命中 gemma-4-12b-qat 拒绝加载
...
4. 对话开始时 signal_pull 拉未消费关怀信号——**信号消费=主动关怀，谁消费谁标记**：signal_claim 原子认领 → 关怀用户 → signal_ack 回执（认领失败=已被其他 agent 消费，跳过即可）。
