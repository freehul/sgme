...
## 背景
排查 VPS（<VPS_IP>）流量暴增：确认无盗用，连接 100% 来自用户本人 IP；陌生 IP 均为未通过 Reality 认证的端口扫描，零实际流量。顺带发现 SSH 密码登录实际生效（被 50-cloud-init.conf 覆盖）、8458 次爆破记录。

## 变更清单（已执行并验证）
1. **SSH 登录用户 root → <USER>**
   - 新增系统用户 <USER>（uid=1000），加入 sudo 组，配置 NOPASSWD 免密 sudo
   - 公钥沿用原 root 的 authorized_keys（密钥文件 ~/.ssh/vps43_ed25519 不变）
   - 本地 ~/.ssh/config 的 vps 别名 User 已改为 <USER>
   - **连接方式：ssh vps（= <USER>@<VPS_IP>:2222，密钥 vps43_ed25519）**
   - root 直连已被拒（Permission denied publickey），勿再用 root@<VPS_IP>
2. **禁用密码登录**
...
## 注意
- <USER> 的 sudo 为 NOPASSWD:ALL：拿到 <USER> 密钥 = 拿到 root，密钥须严格保密
